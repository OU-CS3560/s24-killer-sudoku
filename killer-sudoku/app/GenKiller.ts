...
export class killerBoard {
    ...
    constructor () {
        ...
        for (let i = 0; i < 9; i++) {
            this.at[i] = [];
            for (let j = 0; j < 9; j++) {
                this.at[i][j] = {sum: -1, curSize: 0, maxSize: 0, symbol: '?'};
            }
        }
    }
    ...
    */
    genKiller (tiles: number[][]): void {
        ...
        for (let numOcc = 0; numOcc < 81;) {
            ...
            } while (this.at[x][y].symbol != '.');
            this.at[x][y].symbol = kKey[numGroups];
            numOcc++; numGroups++;
            const borders: [number,number][] = [[x,y-1],[x+1,y],[x,y+1],[x-1,y]];
            for (let [x0,y0] of borders) {
                if (!onBoard(x0,y0)) continue;
                if (this.at[x0][y0].symbol == '.') {this.at[x0][y0].symbol = '/'; numOcc++;}
            }
        }
        ...
        for (let numBlank = 81-numGroups; numBlank > 0;) {
            for (let x = 0; x < 9; x++) {
                for (let y = 0; y < 9; y++) {
                    if (this.at[x][y].symbol != '.' && this.at[x][y].symbol != '/') continue;
                    ...
                    if (avaNeigh.length == 0) {
                        this.at[x][y].symbol = kKey[numGroups];
                        this.at[x][y].maxSize = randMaxSize();
                        numGroups++; numBlank--;
                        continue;
                    }
                    const [a,b] = avaNeigh[rand(0,avaNeigh.length-1)];
                    this.at[a][b].curSize++;
                    this.at[a][b].sum += this.at[x][y].sum;
                    this.at[x][y] = this.at[a][b]; //merged, both locations now point to same tile/info
                    numBlank--;
                }
            }
        }
        ...
        for (let x = 0; x < 9 && numOf1Groups > 3; x++) {
            for (let y = 0; y < 9 && numOf1Groups > 3; y++) {
                if (this.at[x][y].curSize != 1) continue;
                ...
                const [a,b] = avaNeigh[rand(0,avaNeigh.length-1)];
                this.at[a][b].curSize++;
                if (this.at[a][b].curSize > this.at[a][b].maxSize) this.at[a][b].maxSize++;
                this.at[a][b].sum += this.at[x][y].sum;
                this.at[x][y] = this.at[a][b]; //merged, both locations now point to same tile/info
                numOf1Groups--;
            }
     ...
     * @returns None
     */
    doKillerUIStuff(board: SpaceButtonProperties[][]): void {
        ...
        for (let y = 0; y < 9; y++) {
            for (let x = 0; x < 9; x++) {
                const char = this.at[x][y].symbol;
                if (keyTrack[char] == false) {
                    keyTrack[char] = true;
                    board[x][y].topLeftNumber = this.at[x][y].sum;
                }
            }
        }
        ...
        for (let x = 0; x < 9; x++) {
            for (let y = 0; y < 9; y++) {
                ...
                const thisSym: string = this.at[x][y].symbol;
                for (let opt of [[x,y-1],[x+1,y],[x,y+1],[x-1,y]]) {
                    const x0 = opt[0], y0 = opt[1];
                    neighbors.push((onBoard(x0,y0) && this.at[x0][y0].symbol == thisSym) ? '1' : '0');
                }
